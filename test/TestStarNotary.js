...
it('lets 2 users exchange stars', async() => {
    ...
    let instance = await StarNotary.deployed();
    const [tokenId1, tokenId2] = [7, 8];
    const [user1, user2] = [accounts[1], accounts[2]];
...
it('lets a user transfer a star', async() => {
    ...
    await instance.createStar('Yet another star!', tokenId, {from: user});
    // 2. use the transferStar function implemented in the Smart Contract
    const user2 = accounts[1];
    await instance.transferStar(user2, tokenId, {from: user});
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    const tokenId = 10;
    const user = accounts[0];
    const starName = 'Another one!'
    await instance.createStar(starName, tokenId, {from: user});
    // 2. Call your method lookUptokenIdToStarInfo
    const retrievedStarName = await instance.lookUptokenIdToStarInfo.call(tokenId);
